...
- Keep each change **focused on the requested fix** — no unrelated cleanup,
  refactoring, or scope expansion riding along.
- Do **not** edit or re-sort `MANIFEST` by hand — it is regenerated by
  `make manifest` from `MANIFEST.SKIP`. Add new files to the distribution by
  adjusting `MANIFEST.SKIP` patterns, not by hand-editing `MANIFEST`.
- Bug-fix and feature PRs must **never** touch `$VERSION` or `Changes` —
  versioning and changelog updates are a maintainer-only release step (below).

## Release process (maintainer-only)

This dist has two independently versioned modules — `Tie::DBI` (`$VERSION` in
`lib/Tie/DBI.pm`) and `Tie::RDBM` (`$VERSION` in `lib/Tie/RDBM.pm`). The
distribution version (what CPAN sees) is `VERSION_FROM => lib/Tie/DBI.pm` in
`Makefile.PL`, so the dist version tracks `Tie::DBI`. Bump the module(s) you
actually changed; bump `Tie::DBI` for any release since it sets the dist
version. Do not trust `MYMETA.*` or `Makefile` — those are regenerated from
`$VERSION` by `perl Makefile.PL`.

CPAN/PAUSE versioning conventions to follow:

- **Trailing-zero versions are reserved** — don't release a version whose final
  digit is `0` (e.g. `1.10` → `1.11`, not `1.10` again or a `1.090`-style
  trailing zero).
- **Developer / trial releases use a CPAN underscore version** (e.g. `1.09_01`).
  PAUSE recognizes the underscore and treats the upload as a developer release
  that is not indexed as the latest stable — use it for test uploads before
  cutting the real version. Write the same underscore string into the
  version-bearing module.
- Append a new `Changes` entry dated in **ISO format (`YYYY-MM-DD`)**,
  categorized as **`[Bug Fixes]` / `[Improvements]` / `[Maintenance]`**, each
  item carrying `GH #NNN` / `PR #NNN` / `RT #NNN` references so MetaCPAN
  auto-links them. (The existing 1.09 entry is the template to match.)
